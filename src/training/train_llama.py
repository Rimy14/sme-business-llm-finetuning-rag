"""
Llama 3 8B Instruct SFTTrainer Pipeline (Tesla T4 Optimized)
----------------------------------------------------------
Task: KAN-21 (Day 3)
...
def format_llama_chat(example: dict, tokenizer) -> str:
    """Formats an SME sample into Llama 3 Chat template."""
...
def run_llama_training(config_path: str, hf_token: str = None, max_train_samples: int = None, max_val_samples: int = None, use_wandb: bool = False):
    """Executes SFTTrainer fine-tuning for Llama-3-8B on SME dataset."""
    ...
    model_id = cfg.get("model_name_or_path", "meta-llama/Meta-Llama-3-8B-Instruct")
    ...
    output_dir = cfg.get("output_dir", "models/v1/llama_sme_v1")
    ...
    # 4. Format Prompts
    train_texts = [format_llama_chat(x, tokenizer) for x in train_raw]
    val_texts = [format_llama_chat(x, tokenizer) for x in val_raw]
    ...
    logger.info("✅ Llama 3 8B SME Training Completed Successfully!")
    if use_wandb:
        wandb.finish()


if __name__ == "__main__":
    cfg_file = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "configs", "llama_sme_config.yaml")
    run_llama_training(cfg_file)
